...
ConnectionOptions = require("tls").ConnectionOptions;
...
declare module "node-imap" {
  namespace Connection {
    ...
    interface Box {
      ...
      uidnext: number;
      /**
       * array - A list of system-defined flags applicable for this mailbox.
       * Flags in this list but not in permFlags may be stored for the current session only.
      ...
      permFlags: string[];
      /**
       * Whether or not this mailbox has persistent UIDs.
       * This should almost always be true for modern mailboxes and should only be false for
       * legacy mail stores where supporting persistent UIDs was not technically feasible.
       */
  ...
  class Connection extends EventEmitter implements Connection.MessageFunctions {
     ...
     * If autoExpunge is true, any messages marked as Deleted in the currently open mailbox will be removed,
     * if the mailbox was NOT opened in read-only mode. If autoExpunge is false, you disconnect, or you open another mailbox,
     * messages marked as Deleted will NOT be removed from the currently open mailbox.
     */
    ...
    /**
     * Renames a specific mailbox that exists on the server.
     * Both oldMailboxName and newMailboxName should include any necessary prefix/path.
     ...
     * that both have their uid in uids and have the \Deleted flag set.
     * Note: At least on Gmail, performing this operation with any currently open mailbox that is not
